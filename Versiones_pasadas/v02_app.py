...
if opcion == "Inicio":
    ...
    if st.session_state.editar_maquina_id is not None:
        ...
        with st.form("form_editar_maquina"):
            tipo = st.selectbox("Tipo de máquina",TIPOS_MAQUINA,index=TIPOS_MAQUINA.index(maquina_editar[1]) if maquina_editar[1] in TIPOS_MAQUINA else 0)
            ...
            if submitted:
                ...
                actualizar_estado_maquina(st.session_state.editar_maquina_id, estado_operacion)
    ...
    tipos = sorted(list(set([m[1] for m in maquinas])))
    ...
    if tipo_seleccionado != "Seleccione una opción":
        ...
        maquinas_dict = {f"{m[2]} - {m[1]}": m[0] for m in maquinas_filtradas}
    ...
    if not maquinas:
        ...
        ciudades = sorted(list(set([m[7] for m in maquinas if m[7] is not None])))
        ...
        maquinas_ciudad = [m for m in maquinas if m[7] == ciudad]

        # -------------------------
        # FILTRO POR SEDE
        # -------------------------

        sedes = sorted(list(set([m[6] for m in maquinas_ciudad if m[6] is not None])))
        ...
        if sede != "Seleccione una opción":
            sede_sel = sede
        maquinas_sede = [m for m in maquinas_ciudad if m[6] == sede]
        ...
        maquinas_dict = {f"{m[2]} - {m[1]}": m for m in maquinas_sede}
    ...
    maquina_id_seleccionada = st.selectbox(
        ...
        format_func=lambda x: (
            ...
            if x is None
            else f"{maquinas_dict[x][2]} - {maquinas_dict[x][1]}"
        ),
        key="sol_maquina"
    )
    ...
    with st.form(f"form_mantenimiento_{st.session_state.form_mantenimiento_key}"):
        ...
        estado_maquina = st.selectbox(
            ...
            ESTADOS_OPERACION,
            index=ESTADOS_OPERACION.index(maquina[5])
        )
        ...
        if submitted:
            ...
            st.session_state.mantenimiento_registrado = True
            st.session_state.form_mantenimiento_key += 1
            st.rerun()
            
    if "mantenimiento_registrado" in st.session_state:
        st.success("Mantenimiento registrado y solicitudes cerradas.")
        del st.session_state.mantenimiento_registrado
